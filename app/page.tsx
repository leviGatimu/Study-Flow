...
export default async function Dashboard() {
  ...
  return (
    <div className="flex flex-col space-y-12 max-w-[1600px] mx-auto animate-in fade-in duration-500 pb-16">
      {/* Hero Section - Sticky */}
      <section className="sticky top-0 z-30 bg-background/80 backdrop-blur-md flex flex-col xl:flex-row xl:items-center justify-between gap-8 px-4 md:px-8 pt-10 pb-6 border-b border-border/40 mb-2">
        <div className="space-y-4 flex-1">
          <div className="flex items-center gap-6 flex-wrap">
            <DynamicGreeting name={userProgress?.name || 'Student'} />
            
            <div className="h-10 w-px bg-border/60 hidden md:block" />

            {/* Duolingo-style Streak Widget */}
            <div className="flex items-center gap-3 bg-orange-500/10 border border-orange-500/20 px-5 py-2.5 rounded-2xl animate-in zoom-in duration-500 delay-200 shadow-sm shadow-orange-500/5 hover:scale-105 transition-transform cursor-default group">
            ...
            <div className="flex items-center gap-3 bg-blue-500/10 border border-blue-500/20 px-5 py-2.5 rounded-2xl animate-in zoom-in duration-500 delay-300 shadow-sm shadow-blue-500/5 hover:scale-105 transition-transform cursor-default group">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-6">
          ...
          {userProgress && (
            <div className="bg-card border border-border/60 p-5 rounded-[28px] shadow-sm min-w-[280px] hover:shadow-md transition-shadow">
               <RankBadge level={userProgress.level} xp={userProgress.xp} showProgress={true} />
            </div>
          )}
          
          <div className="bg-card border border-border/60 p-5 rounded-[28px] shadow-sm min-w-[220px]">
            <RwandaClock />
          </div>
        </div>
      </section>
