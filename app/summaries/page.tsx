  ...
  return (
    <div className="space-y-12 max-w-[1400px] mx-auto animate-in fade-in duration-500 pb-16 px-4 md:px-8">
      <div className="pt-10 pb-6 border-b border-border/40">
        <h1 className="text-5xl font-heading font-black tracking-tight text-foreground">Analytics</h1>
        <p className="text-xl text-muted-foreground font-semibold mt-3">High-level performance metrics and weekly grades.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {summaries.length === 0 ? (
          ...
          summaries.map(summary => (
            <SummaryCard key={summary.id} summary={summary as WeeklySummary} />
          ))
        )}
      </div>
    </div>
